...
class xajaxResponse
{
	...
	function getOutput()
	{
		...
		// KBP fix to strip html for Unknown Function 
		if(strpos($xml, '[CDATA[Unknown Function') !== false) {
			$search = "<\!\[CDATA\[(.*?)\]\]>";
			// $xml = preg_replace("<\!\[CDATA\[(.*?)\]\]>/e", "'<![CDATA[' . htmlentities('\\1', ENT_QUOTES) . ']]>'", $xml);
			preg_match($search, $xml, $match);
			if(!empty($match[1])) {
				$xml = str_replace($match[1], htmlentities($match[1], ENT_QUOTES), $xml);
			}
		}
