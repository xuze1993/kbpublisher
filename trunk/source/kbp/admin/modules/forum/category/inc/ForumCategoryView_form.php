...
class ForumCategoryView_form extends AppView
{
    var $tmpl = 'form.html';
    ...
    function execute(&$obj, &$manager) {
        ...
        $tpl->tplAssign('category_popup_block', 
            CommonCategoryView::getCategoryPopupBlock($this->controller, $this->msg, 'forum', 'forum_category'));
        
        
        //$select->setMultiple(5, false);
        $select->setSelectName('parent_id');
        $use_sections = SettingModel::getQuick(600, 'forum_sections');
        $top_level_msg_key = ($use_sections) ? 'top_level_section_msg' : 'top_level_msg';
        $select->setRange($range, array(0=>$this->msg[$top_level_msg_key]));
