...
class ForumCategoryView_list extends AppView
{
    ...
    function execute(&$obj, &$manager) {
        ...
        // bulk
        $manager->bulk_manager = new ForumCategoryModelBulk();
        if($manager->bulk_manager->setActionsAllowed($manager, $manager->priv)) {
            $tpl->tplSetNeededGlobal('bulk');
            $tpl->tplAssign('footer', 
                $this->controller->getView($obj, $manager, 'ForumCategoryView_bulk', $this));
        }        
        ...
        $num_messages = ($ids) ? $manager->getMessagesNum($ids) : 0;
        ...
        foreach(array_keys($tree_helper) as $cat_id) {
            ...
            $v['num_messages'] = (isset($num_messages[$cat_id])) ? $num_messages[$cat_id] : '';
            $v['entry_link'] = $this->getLink('forum', 'forum_entry', false, false, array('filter[c]'=>$cat_id));
			$v['top_category_link'] = $this->getLink('forum', 'forum_category', false, false, array('filter[c]'=>$cat_id));
