<?php
/*
    This file will overwrite original config at
    /admin/tools/ckfinder/config.php
    It should be included at the and of original config file
    require_once "../../../../ckeditor_custom/ckfinder_config.php";
*/
...
function CheckAuthentication() {
    ...
    return (AuthPriv::isAuthStatic($conf['auth_check_ip']) && AuthPriv::getPrivId()); 
}

// LicenseKey : Paste your license key here. If left blank, CKFinder will be
// fully functional, in demo mode.
$config['LicenseName'] = 'KBPublisher';
$config['LicenseKey'] = 'VJAV-NSXG-CYX6-RJPG-U5GT-PYVV-CTCE';
...
*/
// $baseDir = resolveUrl($baseUrl);
$baseDir = $setting['html_editor_upload_dir'];
...
include_once '../../../../ckeditor_custom/plugins/kbp_safe_delete/plugin.php';
include_once '../../../../ckeditor_custom/plugins/kbp_safe_rename/plugin.php';


class CKFinderPluginCommon 
{
    
    static function getImageUsageValidationList() {
        
        $list = array(
            array('name' => 'Articles', 'table' => 'kb_entry', 'field' => 'body'),
            array('name' => 'News', 'table' => 'news', 'field' => 'body'),
            array('name' => 'Glossary', 'table' => 'kb_glossary', 'field' => 'definition'),
            array('name' => 'Article Templates', 'table' => 'article_template', 'field' => 'body'),
            array('name' => 'Custom Field', 'table' => 'custom_field', 'field' => 'html_template'),
        );
    ...
    static function getImageUsage($path) {
        
        $reg =& Registry::instance();
        $conf =& $reg->getEntry('conf');
        $db =& $reg->getEntry('db');
        
        $sql_str = "SELECT * FROM %s%s WHERE %s LIKE '%%%s%%'";
        $data = array();
        
        $list = CKFinderPluginCommon::getImageUsageValidationList();
        foreach ($list as $k => $v) {
            $sql = sprintf($sql_str, $conf['tbl_pref'], $v['table'], $v['field'], addslashes($path));
            $result = $db->Execute($sql) or die($sql);
            
            $entries = $result->GetAssoc();
            if (!empty($entries)) {
                $data[$k] = $entries;
            }
        }
