...
--
ALTER TABLE `kbp_news` ADD `hits` INT UNSIGNED NOT NULL DEFAULT '0' AFTER `body`;


--
ALTER TABLE `kbp_file_entry` ADD `filename_index` VARCHAR(256) NOT NULL AFTER `filename`;
--
UPDATE `kbp_file_entry` SET `filename_index` = LOWER(REPLACE(LEFT(filename, LOCATE('.', filename)-1), '_', ' ')), `date_updated` = date_updated;
--
ALTER TABLE `kbp_file_entry` DROP INDEX `title`, ADD FULLTEXT `title` (`title`,`filename_index`,`description`,`filetext`);
